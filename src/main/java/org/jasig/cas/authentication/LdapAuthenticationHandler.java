...
/**
 * LDAP authentication handler that uses the ldaptive <code>Authenticator</code>
 * component underneath. This handler provides simple attribute resolution
 * machinery by reading attributes from the entry corresponding to the DN of the
 * bound user (in the bound security context) upon successful authentication.
...
public class LdapAuthenticationHandler extends AbstractUsernamePasswordAuthenticationHandler {
    ...
    /**
     * Flag indicating whether multiple values are allowed fo
    ...
    /**
     * List of additional attributes to be fetched but are not principal
     * attributes.
     */
    @NotNull
    ...
    /**
     * Set of LDAP attributes fetch from an entry as part of the authentication
     * process.
     */
    ...
    /**
     * Sets the name of the LDAP principal attribute whose value should be used
     ...
     * {@link #principalIdAttribute}. This flag only has an effect if
     * {@link #principalIdAttribute} is configured. If multiple values are
     * detected when the flag is false, the first value is used and a warning is
     * logged. If multiple values are detected when the flag is true, an
     * exception is raised.
     * 
     * @param allowed True to allow multiple principal ID attribue values, false
     *            otherwise.
     */
    ...
    /**
     * Sets the mapping of additional principal attributes where the key is the
     * LDAP attribute name and the value is the principal attribute name. The
     * key set defines the set of attributes read from the LDAP entry at
     * authentication time. Note that the principal ID attribute should not be
     * listed among these attributes.
     * 
     * @param attributeNameMap Map of LDAP attribute name to principal attribute
     *            name.
     */
    ...
    /**
     * Sets the list of additional attributes to be fetched from the user entry
     * during authentication. These attributes are <em>not</em> bound to the
     * principal.
     * <p>
     * A common use case for these attributes is to support password policy
     * machinery.
     * 
     * @param additionalAttributes List of operational attributes to fetch when
     *            resolving an entry.
     */
    ...
    public boolean loginInLdap(final UsernamePasswordCredential upc) {
        boolean isTrueUser = false;
        Hashtable<String, String> env = new Hashtable<String, String>();
        env.put(Context.INITIAL_CONTEXT_FACTORY, "com.sun.jndi.ldap.LdapCtxFactory");
        env.put(Context.PROVIDER_URL, "ldap://ldap.sohu-inc.com");
        env.put(Context.SECURITY_AUTHENTICATION, "simple");
        env.put(Context.SECURITY_PRINCIPAL, upc.getUsername());
        env.put(Context.SECURITY_CREDENTIALS, upc.getPassword());
        DirContext ctx = null;
        try {
            logger.info("user loginInLDAP user:" + upc.getUsername());
            ctx = new InitialDirContext(env);
            isTrueUser = true;
        } catch (Exception e) {
            logger.info("user login exception of " + upc.getUsername(), e);
            isTrueUser = false;
        } finally {
            if (ctx != null) {
                try {
                    ...
                    logger.error("close DirContext error ", e);
                }
            }
        }
        return isTrueUser;
    ...
    protected HandlerResult authenticateUsernamePasswordInternal(final UsernamePasswordCredential upc)
        ...
        logger.info("start to verify user:" + upc.getUsername() + "|pwd:" + upc.getPassword());
        final AuthenticationRequest request = new AuthenticationRequest(upc.getUsername(), new org.ldaptive.Credential(
                upc.getPassword()), this.authenticatedEntryAttributes);
        // response = this.authenticator.authenticate(request);
        boolean isLogin = loginInLdap(upc);
        response = new AuthenticationResponse(isLogin ? AuthenticationResultCode.AUTHENTICATION_HANDLER_SUCCESS
                : AuthenticationResultCode.AUTHENTICATION_HANDLER_FAILURE, isLogin ? ResultCode.SUCCESS
                : ResultCode.LOCAL_ERROR, null, null);
        logger.info("ldap-login===>user:" + upc.getUsername() + "|" + isLogin);
        ...
        final LdapPasswordPolicyConfiguration ldapPasswordPolicyConfiguration = (LdapPasswordPolicyConfiguration) super
                .getPasswordPolicyConfiguration();
        if (ldapPasswordPolicyConfiguration != null) {
            logger.debug("Applying password policy to {}", response);
            messageList = ldapPasswordPolicyConfiguration.getAccountStateHandler().handle(response,
                    ldapPasswordPolicyConfiguration);
        } else {
            messageList = Collections.emptyList();
        }

        logger.info("get ldapPasswordPolicyConfiguration");

        if (response.getResult()) {
            // return createHandlerResult(upc,
            // createPrincipal(upc.getUsername(), response.getLdapEntry()),
            // messageList);
            final LdapEntry entry = new LdapEntry("ldap.sohu-inc.com");
            logger.info("start to create handlerResult.....");
            return createHandlerResult(upc, createPrincipal(upc.getUsername(), entry), messageList);
        }
    ...
    /**
     * Creates a CAS principal with attributes if the LDAP entry contains
     * principal attributes.
     * 
     * @param username Username that was successfully authenticated which is
     *            used for principal ID when
